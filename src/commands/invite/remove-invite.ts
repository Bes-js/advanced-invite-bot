...
export default {
    name: "remove-invite",
    category: "invite",
    description: "Removes Bonus Invite to a User",
    default_member_permissions:[PermissionFlagsBits.ManageGuild.toString()],
    options: [
        {
         name:"user",
         description:"User you want to remove bonus",
         type:6, 
         required:true
        },
        {
        name:"count",
        description:"Bonus Amount You Want to Remove",
        type: 4,
        required:true
        }
    ],
    run: async (client: bot, interaction: CommandInteraction, args: Array<String | CommandInteractionOption>) => {
      ...
      inviteClient.inviteRemove(`${interaction.guild?.id}`,member,count);

      interaction.editReply({embeds:[new EmbedBuilder().setColor("#00ff00").setDescription(`> **Removed \`${count}\` Invites from User ${member}**`)]})
      }catch(err){
